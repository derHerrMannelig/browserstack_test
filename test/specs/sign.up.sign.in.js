...
describe('Sign up / sign in', () => {
    it('should verify registration and login', async () => {
        ...
        await page.touch(loginPage.signUpTab);
        await loginPage.fillCredentials(randomEmail, randomPassword);
        await page.touch(loginPage.btnSignUp);
        await expect(loginPage.alertSuccess).toBeDisplayed();
        await expect(loginPage.alertSuccess).toHaveText('Signed Up!');
        await page.touch(loginPage.btnAlert);
        await page.touch(loginPage.loginTab);
        await page.touch(loginPage.btnLogin);
        await expect(loginPage.alertSuccess).toBeDisplayed();
        await expect(loginPage.alertSuccess).toHaveText('Success');
    })
})
